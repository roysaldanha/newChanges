...
//***
	...
	public function woof_draw_checkbox_taxonomy($taxonomy_info, $tax_slug, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $is_parent_visible, $is_first = 'N') {
    	...
    	if (!empty($childs) AND is_array($childs)) {
    		foreach ($childs as $k => $term) {  $inique_id = uniqid();
                ...
                $is_child_visible = in_array($current_request[0], $uniquechilds)?"YES":"NO";
 ?> 
        	 		<ul class="expand_button_<?php echo $term_id; ?>" 
        	 			style="<?php echo ($is_parent_visible == 'YES' || in_array($current_request[0], $uniquechilds) || $current_request[0] == $term['slug']) || ($term['is_first'] == 'Y' && empty($current_request[0]))?"display:block;":"display:none;"; ?>">
        	 			<li class="container" <?php echo $style; ?>>
						<?php if(!empty($term['childs'])) { ?>
							<?php if((in_array($current_request[0], $uniquechilds) && count($term['childs'])<=1) || ($is_parent_visible == 'YES' && in_array($current_request[0], $uniquechilds)) || ($current_request[0] == $term['slug'] && empty($term['childs'])) ) { ?>
							<a href="javascript: void(0);" class="expand_button poof_is_opened" id="expand_button_<?php echo $term['term_id']; ?>">-</a>
							<?php } else { ?>
							<a href="javascript: void(0);" class="expand_button poof_is_closed" id="expand_button_<?php echo $term['term_id']; ?>">+</a>
							<?php } ?>
						<?php } ?>
						 <a href="<?php echo $shop_page_url; ?>?swoof=1&product_cat=<?php echo $term['slug']; ?>"  <?php echo $current_request[0] == $term['slug']?"style='color:blue'":''; ?>><?php echo $term['name']; ?></a>
						 	<?php if (!empty($term['childs']))
	                        {
	                            $this->woof_draw_checkbox_taxonomy($taxonomy_info, $tax_slug, $term['term_id'], $term['childs'], $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $is_child_visible);
	                        } ?>
                    	</li>
                	</ul> 
            	<?php
	...
	public function woof_draw_checkbox_taxonomy_product($taxonomy_info, $current_request, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $is_parent_visible, $is_first = 'N') {
    	...
    	if (!empty($childs) AND is_array($childs)) {
    		foreach ($childs as $k => $term) {  $inique_id = uniqid();
		        ...
		        $uniquechilds = array_unique($this->tests);
		        if (in_array($current_request, $uniquechilds) || empty($current_request)) { ?>
		            <script type="text/javascript"> 
		            shownLis.push('<?php echo $term['term_id']; ?>');
		            </script>
		            <?php 
                ...
                $is_child_visible = in_array($current_request, $uniquechilds)?"YES":"NO";
 ?> 
        	 		<ul class="expand_button_<?php echo $term_id; ?>" 
        	 			style="<?php echo ($is_parent_visible == 'YES' || in_array($current_request, $uniquechilds) || $current_request == $term['slug']) || ($term['is_first'] == 'Y' && empty($current_request))?"display:block;":"display:none;"; ?>">
        	 			<li class="container" <?php echo $style; ?>>
						<?php if(!empty($term['childs'])) { ?>
							<?php if((in_array($current_request, $uniquechilds) && count($term['childs'])<=1) || ($is_parent_visible == 'YES' && in_array($current_request, $uniquechilds)) || ($current_request == $term['slug'] && empty($term['childs'])) ) { ?>
							<a href="javascript: void(0);" class="expand_button poof_is_opened" id="expand_button_<?php echo $term['term_id']; ?>">-</a>
							<?php } else { ?>
							<a href="javascript: void(0);" class="expand_button poof_is_closed" id="expand_button_<?php echo $term['term_id']; ?>">+</a>
							<?php } ?>
						<?php } ?>
						 <a href="<?php echo $shop_page_url; ?>?swoof=1&product_cat=<?php echo $term['slug']; ?>"  <?php echo $current_request == $term['slug']?"style='color:blue'":''; ?>><?php echo $term['name']; ?></a>
						 	<?php if (!empty($term['childs']))
	                        {
	                            $this->woof_draw_checkbox_taxonomy_product($taxonomy_info, $current_request, $term['term_id'], $term['childs'], $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $is_child_visible);
	                        } ?>
                    	</li>
                	</ul> 
            	<?php
